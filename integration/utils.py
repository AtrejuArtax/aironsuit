...
def test_application(repos_path, application_name):
    ...
    if os.path.isdir(scripts_path):
        script_names = [
            os.path.join(scripts_path, script_name)
...
def test_scripts(script_names, logs_path):
    ...
    for script_name in script_names:
        ...
        while len(arguments_list) > 0:
            ...
            print(
                "{} ({} seconds): {}".format(
                    log_file_name, round(time.time() - start_time), status[1]
                )
            )
...
def get_script_arguments(script_name):
    ...
    if os.path.join("benchmarks", "runner") in script_name:
        arguments_options = [project, precision]
    elif (
        os.path.join("aironsuit", "examples", "standard_classification_pipeline")
        in script_name
        or os.path.join("data_cleaner", "main") in script_name
        or os.path.join("benchmarks", "runner") in script_name
    ):
        arguments_options = [precision, new_design, max_evals, epochs]
    if arguments_options is not None:
        arguments += [
            " ".join(list(args)) for args in list(product(*arguments_options))
        ]
    return arguments
