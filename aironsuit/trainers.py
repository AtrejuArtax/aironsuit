...
def fit(module, x_train, y_train=None, x_val=None, y_val=None, sample_weight=None, sample_weight_val=None,
    ...
    if all([isinstance(data, tf.data.Dataset) for data in training_args]):
        ...
        training_kwargs['validation_data'] = tf.data.Dataset.zip(training_kwargs['validation_data'])
        training_kwargs['validation_data'] = training_kwargs['validation_data'].batch(kwargs['batch_size'])
        if sample_weight is not None:
